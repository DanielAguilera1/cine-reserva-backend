...
public class TiqueteService {
    ...
    public TiqueteService(TiqueteRepository tiqueteRepository, FuncionService funcionService, UsuarioService usuarioService) {
    ...
    public void AgregarTiquete(Tiquete tiquete) throws Exception{
        if (tiquete.getId() != null) throw new Exception("No puedes colocar ID al tiquete");
        if (usuarioService.ObtenerFuncionPorID(tiquete.getUsuarioId()).getId().describeConstable().isEmpty()) throw new Exception("El Usuario no existe");
        if (funcionService.ObtenerFuncionPorID(tiquete.getFuncionId()).getId().isEmpty()) throw new Exception("La Pelicula para esta funcion no existe");
